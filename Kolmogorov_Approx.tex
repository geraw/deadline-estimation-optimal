...
\usepackage[activate={true,nocompatibility},final,tracking=true,kerning=true,spacing=true,factor=1100,stretch=10,shrink=100]{microtype}
...
\DeclareMathOperator{\OptTrim}{OptTrim}
  ...
  \pdfinfo{
	/Title (Optimal Approximation of Discrete Random Variables for Estimation of Probabilities of Missing Deadlines
	)
	/Author ()}
...
%In the scope of this section, let $X$ be a random variable ket $m$ be such that  $0<m\leq |X|$. We present algorithms for finding optimal $m$-approximations of $X$. 
...
\begin{algorithm}
    ...
    $f \gets 0$, $l \gets n$+1\; 
    \lWhile{$c_{f +1} \leq \varepsilon$}{$f \gets f + 1$}
...
\begin{figure}[htb]
	...
	\end{axis}
	\end{tikzpicture}
	\caption{Run-time of a long computation with $\KlmApprox$, $\BinsApprox$, $\OptTrim$, $\Trim$, and without any trimming (exact computation).}
	\label{fig:runtime}
\end{figure}
